...
{	
	//test git
	VideoCapture capture("D:\\AndroidStdio\\lanedetection\\GraduationProject\\测试数据\\road.avi");
	...
	//	cout << "judge"<<(b - a) << "  ";
		//dec.TCLJudge(frame);
		 dec.drawLines(frame) ;
   //imwrite("D:\\AndroidStdio\\lanedetection\\GraduationProject\\测试数据\\road\\" + to_string(count++)+ ".png", frame);
	//	b = clock();
	//	cout << "total"<<(b - a)  << endl;
	}
